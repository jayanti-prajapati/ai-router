...
import { registerProvider, resetProviderRegistry } from '../providers/registry.js';
...
describe('runAiRequest — integration', () => {
  ...
  test('output matches stub response', async () => {
    const result = await runAiRequest({ task: 'format', input: 'convert this' });
    assert.equal(result.output, 'stub response');
  });

  test('simple task routes to simple tier model', async () => {
    const result = await runAiRequest({ task: 'format', input: 'hello', debug: true });
  ...
  test('custom prompt template is used when registered', async () => {
    ...
    assert.equal(result.complexity, 'simple');
    assert.ok(result.debug?.signals.some((s) => s.includes('pinned by prompt')));
  });

  test('priority=speed produces a result', async () => {
    const result = await runAiRequest({ task: 'summarize', input: 'text', priority: 'speed' });
    assert.ok(result.output.length > 0);
  });

  test('priority=quality produces a result', async () => {
    const result = await runAiRequest({ task: 'explain', input: 'What is recursion?', priority: 'quality' });
    assert.ok(result.output.length > 0);
  });
  ...
  test('quality escalation fires when stub returns low-quality output', async () => {
    ...
    const result = await runAiRequest({ task: 'explain', input: 'explain recursion' });
